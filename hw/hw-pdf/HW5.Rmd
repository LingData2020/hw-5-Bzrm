...
library('dplyr')
attach(shva)
#shva %>% select (f1, f2) -> f1_f2_scorec 
plot(f1, f2, main="Scatterplop",
   xlab="f1 ", ylab="f2", pch=19)
```
...
```{r 1.2}
# f1 boxplot
ggplot(shva, aes(x=vowel, y=f1)) + geom_boxplot(width=0.3) + coord_flip()
# f2 boxplot
ggplot(shva, aes(x=vowel, y=f2)) + geom_boxplot(width=0.3) + coord_flip()
```
...
```{r 1.3}
cor.test(x = shva$f1, y = shva$f2, method = 'pearson')
```
### 
1.4 Calculate Pearson's correlation of `f1` and `f2` for each vowel
```{r 1.4}
shva %>% filter(vowel == 'A') -> shva_vowels_A
shva %>% filter(vowel == 'a') -> shva_vowels_a
shva %>% filter(vowel == 'y') -> shva_vowels_y
```

```{r}
cor.test(x = shva_vowels_A$f1, y = shva_vowels_A$f2, method = 'pearson')
cor.test(x = shva_vowels_a$f1, y = shva_vowels_a$f2, method = 'pearson')
cor.test(x = shva_vowels_y$f1, y = shva_vowels_y$f2, method = 'pearson')
```
```{r}
shva %>%
  group_by(f1, f2) %>%
  summarise(vowel) -> vowel_shva
cor.test(x = vowel_shva$f1, y = vowel_shva$f2, method = 'pearson')
```
...
```{r 2.1.4}
ggplot(data = f1_f2_scores, aes(x = f1, y = f2)) + 
  geom_point() +
  labs(x = "f1",
       y = "f2",
       title = "Scores") +
...
```{r 2.1.5}
  shva %>% 
  ggplot(aes(f1, f2))+ geom_point(aes(color=vowel)) + geom_smooth(method="lm")
```
...
```{r 2.2.1}
model2 <- lm(data = vowel_shva, f2 ~ f1 + vowel)
summary(model2)
```
...
```
f1 and f2 have negative correlation which means if f1 gets higher f2 at the same time gets lower. Considering vowels (plot in 2.1.5.) 'a' vowel has got somewhat equal results on f1 and f2 while 'A' showed higher f1 score with lower f2 score and 'y' showed higher f2 and lower f1. So overall f1 and f2 have negative correlation.  
```
...
chisq.test(x = d_caus$Aux, y = d_caus$CrSem)
chisq.test(x = d_caus$Aux, y = d_caus$CeSem)
```
```Association between Aux and CeSynt and association between Aux and CrSem are statistically significant (p-value is lower than 0.05)```
...
```{r}
obs_tr <- chisq$observed
obs_tr
```


```{r 3.4}
obs_or <- (57/182)/(28/233)
obs_or
```
...
```{r 3.4.1}
exp_or <- (40.63/198.37)/(44.37/216.63)
exp_or
```

What can you say about odds ratio of expected frequencies for arbitrary data?

```It equals 1 while observed odds ratio equals 2.606162, which means that there are correlation between two variables (Intr label of EPTrans raises odds of Doen) ```
...
```
We have  found a significant association between variables Aux and EPTrans (p < 0.001).  The odds of 'Doen' were 2 times higher in (group observed) than in (group expected). However effect size is large (Cramer's V = 0.1691587).
```
...
```
Chisq is only used when there are only 2 categorical variables, otherwise variables can be correlated between each other (because they came from one source), so adjustment is needed.
```

### 3.9 
Provide a short text (300 words) describing the hypothesis of this study and the results of your analysis.
```{r 3.9}
This study was contributed to question "Which factors affect subjective experience from semantically related aux verbs doen and laten". For this study linguistic and pragmatic factors were chosen, however in our particular case we concentrated only on linguicstic factors. Our test showed that there are significant association between Aux and CeSynt and also between Aux and CrSem variables. Also Auxiliary verbs are correlated with transitivity(EPTrans variable; intransitive label raises the odds for "doen"). However this last conclusion could be possibly caused by corpus-size (look at cramer`s V in 3.5).  
```